...
def pos_galaxia_espiral(
    i: int, n: int, centro: np.ndarray, radio: float, rotacion: float, seed: int
) -> np.ndarray:
...
def pos_corazones_entrelazados(i: int, n: int) -> np.ndarray:
    ...
    return np.array([hx * escala + desplaz_x, hy * escala * 0.85 + desplaz_y, 0])
...
class Animacion(Scene):
    ...
    def construct(self):
        ...
        colores_init = [
            nebula_color(rng.random(), int(rng.integers(0, 2))) for _ in range(n)
        ]
        ...
        pos_corazones = [pos_corazones_entrelazados(i, n) for i in range(n)]
        ...
        def update_escena(mob, dt):
            ...
            for i, dot in enumerate(mob):
                ...
                if t <= T_CAOS_FIN:
                    ...
                    dot.set_color(nebula_color(abs(np.sin(t * 0.3 + i * 0.1)), lado))
                ...
                # ── Fase 2: Formación galaxias (25–70s) ───────────────────
                    ...
                    dot.set_color(
                        nebula_color(min((j / n2) * 0.8 + prog * 0.2, 1.0), lado)
                    )
                ...
                # ── Fase 3: Órbita y acercamiento (70–120s) ───────────────
                    ...
                    if t >= T_TEXTO_INICIO:
                        texto_mob.set_opacity(min((t - T_TEXTO_INICIO) / 3.0, 0.85))
                ...
                # ── Fase 5: Corazones entrelazados (140–258s) ─────────────
                    ...
                    dot.set_color(
                        nebula_color(abs(np.sin((i / n) * 0.9 + t * 0.12)), lado)
                    )
                    ...
                    if t <= T_TEXTO_FIN:
                        ...
                        titulo_final.set_opacity(min((t - T_TEXTO_FIN) / 4, 0.8))
